...
begin
    if (edtDataFinal.Date) < (edtDataInicio.Date) then begin
      MessageDlg('Data Final não pode ser menor do que a data início', TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0);
      edtDataFinal.SetFocus;
      Abort;
    end;
    if (edtDataFinal.Date = 0) OR (edtDataInicio.Date = 0) then begin
      MessageDlg('Data Inicial ou final são campos obrigatórios', TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0);
