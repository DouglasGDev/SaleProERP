...
type
  ...
  published
    ...
    property Pix:Double          read F_pix        write F_pix;
    property Credito:Double      read F_credito    write F_credito;
    property Debito:Double       read F_debito     write F_debito;
    property Dinheiro:Double     read F_dinheiro   write F_dinheiro;
...
begin
  try
    ...
    Qry.ParamByName('pix').AsFloat          :=Self.F_pix;
    Qry.ParamByName('credito').AsFloat      :=Self.F_credito;
    Qry.ParamByName('debito').AsFloat       :=Self.F_debito;
    Qry.ParamByName('dinheiro').AsFloat     :=Self.F_dinheiro;
...
begin
  try
    ...
    Qry.SQL.Clear;
    Qry.SQL.Add('INSERT INTO vendas (clienteId, pix, credito, debito, dinheiro, dataVenda, totalVenda)  '+
                '            VALUES (:clienteId, :pix, :credito, :debito, :dinheiro, :dataVenda,:totalVenda )');
    Qry.ParamByName('clienteId').AsInteger  :=Self.F_clienteId;
    Qry.ParamByName('pix').AsFloat          :=Self.F_pix;
    Qry.ParamByName('credito').AsFloat      :=Self.F_credito;
    Qry.ParamByName('debito').AsFloat       :=Self.F_debito;
...
begin
  try
    ...
    Try
      ...
      Self.F_pix        := Qry.FieldByName('pix').AsFloat;
      Self.F_credito    := Qry.FieldByName('credito').AsFloat;
      Self.F_debito     := Qry.FieldByName('debito').AsFloat;
      Self.F_dinheiro   := Qry.FieldByName('dinheiro').AsFloat;
