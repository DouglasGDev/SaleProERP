...
begin
   ...
   try
     ...
     Qry.ParamByName('fornecedorId').AsInteger :=F_fornecedorId;// aqui armazena a informação no parametro com o tipo permitido
     Try
...
begin
  try
    ...
    Qry.Connection:=ConexaoDB;// aqui recebe a conexão
    Qry.SQL.Clear;// limpa pra evitar impureza
    Qry.SQL.Add('UPDATE fornecedores ' +
    ...
    Qry.ParamByName('fornecedorId').AsInteger    :=Self.F_fornecedorId;// aqui armazena a informação no parametro com o tipo permitido
...
begin
  try
    ...
    Qry.Connection:=ConexaoDB;// aqui recebe a conexão
    Qry.SQL.Clear;// limpa pra evitar impureza
    Qry.SQL.Add('INSERT INTO fornecedores (razaosocial ' +
                 '    ,fantasia ' +
                 '    ,cnpj '+
                 '    ,endereco ' +
                 '    ,bairro   ' +
...
begin
  try
    ...
    Qry.Connection:=ConexaoDB;// aqui recebe a conexão
    Qry.SQL.Clear;// limpa pra evitar impureza
    Qry.SQL.Add('SELECT fornecedorId, '+
    '                  razaosocial ' +
                 '    ,fantasia ' +
                 '    ,cnpj '+
                 '    ,endereco ' +
    ...
    Qry.ParamByName('fornecedorId').AsInteger:=id;// aqui armazena a informação no parametro que pega do id la encima do selecionar
    try
      Qry.Open;// tenta abrir a conexão
      Self.F_fornecedorId       := Qry.FieldByName('fornecedorId').AsInteger;// pega o id do campo(interface) e atribui a query com o campo fornecedorid e com o tipo em inteiro
