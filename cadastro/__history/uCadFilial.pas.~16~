...
begin
  inherited;
   if Assigned(oFilial) then
     FreeAndNil(oFilial); // Verifica se tem alguma variavel instanciada na memoria e limpa da memoria a informação
end;
...
begin
  ...
  if edtFilialId.Text<>EmptyStr then // se o campo do id for diferente de vazio
         oFilial.codigo:=StrToInt(edtFilialId.Text)// atribui pro campo código o texto convertido em numero inteiro
  else
     oFilial.codigo:=0;
