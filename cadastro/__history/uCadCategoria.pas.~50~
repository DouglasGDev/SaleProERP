...
begin
  if edtCategoriaId.Text<>EmptyStr then // se o campo do id for diferente de vazio
     oCategoria.codigo:=StrToInt(edtCategoriaId.Text)// atribui pro campo código o texto convertido em numero inteiro
  else
     oCategoria.codigo:=0;
...
begin
  ...
  if Assigned(oCategoria) then
     FreeAndNil(oCategoria); // Verifica se tem alguma variavel instanciada na memoria e limpa da memoria a informação
end;
