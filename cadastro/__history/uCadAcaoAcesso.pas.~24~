...
begin
   if oAcaoAcesso.Selecionar(QryListagem.FieldByName('acaoAcessoId').AsInteger) then begin // apaga pelo campo acaoAcessoId(id) que está pegando do db que manda o id pra classe que já trata e faz a exclusão
...
begin
  ...
  if oAcaoAcesso.ChaveExiste(edtChave.Text, oAcaoAcesso.codigo) then begin // verifica se ja existe a chave, se existir ele da um foco na chave e aborta.
    MessageDlg('Chave já cadastrado', TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK],0);
    edtChave.SetFocus;
    abort;
  end;
